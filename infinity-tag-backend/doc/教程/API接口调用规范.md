...
从 v1.1 开始，支持用户上传自定义图片并进行裁剪、二值化处理，适配 2.13寸 墨水屏（212x104）。
...
**响应**: `image/png` 二进制流（212x104，1-bit）
...
为了获得最佳显示效果，建议前端：
1. 使用裁剪工具时锁定宽高比为 **212:104** (约 2.04:1)
2. 裁剪后的内容会被强制缩放到 212x104，保持比例可避免拉伸变形
